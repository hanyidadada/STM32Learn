...
void DEBUG_USART_IRQHandler(void)
{
  ...
  if (USART_GetITStatus(DEBUG_USARTx, USART_IT_RXNE) != RESET) {
    ...
    if (USART_data.flags.len < MAXLEN - 1) {
      USART_data.databuff[USART_data.flags.len++] = temp; 
    }
  }

  if (USART_GetITStatus(DEBUG_USARTx, USART_IT_IDLE) == SET) {
//		printf("\n\r\rUSART:%s \n", USART_data.databuff);
    USART_data.flags.flag = 1;
    temp = USART_ReceiveData(DEBUG_USARTx);
  }
}
...
void ESP8266_IRQHandler(void)
{
  ...
  if (USART_GetITStatus(ESP8266_USARTx, USART_IT_RXNE) != RESET) {
    ...
    if (Esp8266_data.flags.len < MAXLEN - 1) {
      Esp8266_data.databuff[Esp8266_data.flags.len++] = temp; 
    }
  }

  if (USART_GetITStatus(ESP8266_USARTx, USART_IT_IDLE) == SET) {
    Esp8266_data.flags.flag = 1;
//		printf("\n\r ESP:%s \n", Esp8266_data.databuff);
    temp = USART_ReceiveData(ESP8266_USARTx);
  }
}
